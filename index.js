...
/* =========================================================
   ردود افتراضية لائقة - بترجع لو مفيش ولا مثال متعلَّم أصلاً
   ولا حتى DuckDuckGo رجّع حاجة مفيدة.
   ========================================================= */
const DEFAULT_REPLY = 'أهلاً بك! لسه بتعلّم، تقدر تدّيني مثال أو تصحّحلي عشان أفهم أكتر 🙂';

/* =========================================================
   duckDuckGoFallback: لو مفيش مطابقة واثقة، نجرّب نسأل
   DuckDuckGo Instant Answer API. لو فشل الطلب أو مفيش رد
   مفيد، بترجع null (وده بيتحول تلقائي لـ DEFAULT_REPLY فوق -
   يعني الدالة دي أبداً ما بترجعش undefined لحد استخدامها).
   ========================================================= */
async function duckDuckGoFallback(text){
  try{
    const url = `https://api.duckduckgo.com/?q=${encodeURIComponent(text)}&format=json&no_html=1&skip_disambig=1`;
    const res = await fetch(url);
    if(!res.ok) return null;
    const data = await res.json();
    const candidate = data.AbstractText
      || data.Answer
    ...
    console.error('duckDuckGoFallback failed:', err);
    return null;
  }
}

/* =========================================================
   pickResponse: بتختار رد عشوائي من ردود الفئة (label)، وبترجع
   null بس لو مفيش ردود محفوظة أصلاً - عشان نضمن إننا نرجّع نص
   حقيقي دايماً في classify مش قيمة فاضية.
   ========================================================= */
function pickResponse(label){
  if(!label || !Array.isArray(label.responses) || label.responses.length===0) return null;
  return label.responses[Math.floor(Math.random()*label.responses.length)];
}

/* =========================================================
   classify: المتصفح بينادي عليها بس ويبعت النص، والسيرفر يرجّع
   الرد. المتصفح مش شايل ولا سطر شبكة عصبية خالص.
   شكل الرد ثابت دايماً: { reply: string, confidence: 0-100 }
   - مفيش أي حالة ممكن ترجّع reply=null أو undefined.
   ========================================================= */
exports.classify = onCall(async (request) => {
  ...
  // مفيش أي أمثلة متعلَّمة أصلاً - جرّب DuckDuckGo وإلا رد افتراضي
  if(usable.length===0){
    const fallback = await duckDuckGoFallback(text);
    return { reply: fallback || DEFAULT_REPLY, confidence: 0, feeling };
  }
  ...
  let finalLabel = null, confidence = 0;

  if(simLabel && nnLabel && simLabel.index===nnLabel.index){
    confidence = Math.min(1, bestSim*0.7 + nnConf*0.3 + 0.08);
    if(confidence >= CONFIDENCE_THRESHOLD) finalLabel = simLabel;
  } else if(bestSim >= CONFIDENCE_THRESHOLD){
    finalLabel = simLabel; confidence = bestSim;
  } else if(nnConf >= CONFIDENCE_THRESHOLD+0.15){
    finalLabel = nnLabel; confidence = nnConf;
  }

  if(finalLabel){
    const responseText = pickResponse(finalLabel);
    if(responseText){
      return {
        reply: responseText,
    ...
    // في حالة نادرة: الفئة موجودة بس مفيش ردود محفوظة فيها -
    // بنعتبرها زي عدم الثقة ونكمل على DuckDuckGo/الرد الافتراضي
  }

  // مفيش مطابقة واثقة - جرّب DuckDuckGo، ولو مفيش رد رجّع الافتراضي
  const fallback = await duckDuckGoFallback(text);
  return {
    reply: fallback || DEFAULT_REPLY,
    confidence: Math.round(Math.max(bestSim,0)*100),
    feeling
  };
});
